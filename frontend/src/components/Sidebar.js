...
export default function Sidebar() {
  ...
  const { data: conversations, error, isLoading } = useQuery(['conversations'], fetchConversations);
  ...
  const handleCreateConversation = () => {
    createMutation.mutate({ name: newConversationName, model: 'gpt-3.5-turbo', params: {} });
    setNewConversationName('');
  };

  return (
    <Navbar padding="xs" width={{ base: 300 }}>
      <Button fullWidth onClick={handleCreateConversation}>New Conversation</Button>
